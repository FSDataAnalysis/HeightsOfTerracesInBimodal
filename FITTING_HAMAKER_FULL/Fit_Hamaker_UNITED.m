...
%%%% Corrected indentation S Santos AND Alice C 2015 August 12. Corrected
%%%% for a singlew force, i.e. united all
%%%% distance
...
%     dumb_current_force=sprintf('No_%i', iii);
    ...
    dumb_distance=rolled_vector_distances_ignore(Remove_initial:end-Remove_end);
    dumb_FORCE=FORCE_smooth;
    ...
    distance_raw_nm_cut_off_at_perc_FAD=distance_raw_nm_cut_off(end);
    ...
    if length(linear_distance_raw_nm)> minimu_number_for_fit
        ...
        correction_normalized_indentation=(cut_off_force_min*FORCE_adhesion_nN-beta0)/beta1;
        ...
        matrix_stats_normalized_dumb=[matrix_stats_normalized_dumb, correction_normalized_indentation];
...
add_distace=(correction_normalized_indentation*1e-9-distance_raw_nm_cut_off_at_perc_FAD*1e-9);
plot(dumb_distance, dumb_FORCE, '.r')
plot(d_m,y_m, '-b', 'linewidth', M_size-5, 'Markersize',M_size, 'displayname','H fit');  
